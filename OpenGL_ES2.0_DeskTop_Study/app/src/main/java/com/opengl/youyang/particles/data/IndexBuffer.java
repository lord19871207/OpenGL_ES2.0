...
public class IndexBuffer {
    ...
    public IndexBuffer(short[] indextData){
        ...
        ShortBuffer indexArray= ByteBuffer.allocateDirect(indextData.length * Constants.BYTES_PER_SHORT).order(ByteOrder.nativeOrder())
                .asShortBuffer().put(indextData);

        indexArray.position(0);

        GLES20.glBufferData(GLES20.GL_ELEMENT_ARRAY_BUFFER, indexArray.capacity() * Constants.BYTES_PER_FLOAT, indexArray, GLES20.GL_STATIC_DRAW);
